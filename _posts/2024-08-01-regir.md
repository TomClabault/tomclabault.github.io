...
Let's now introduce Resmapled Importance Sampling (RIS) which is the basis for ReGIR.

Resampled Importance Sampling<d-cite key="RISTalbot"></d-cite> is at the heart of ReGIR. At its core, it is a technique that takes multiple samples $(X_1, ..., X_M)$ produced according to a **source distribution $p$** and "resamples" them
