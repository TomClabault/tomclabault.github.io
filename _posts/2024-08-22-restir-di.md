...
in the scene only effectively sees a fraction of the lights. 
...
[1 SPP the white room lights here]
...
Resampled Importance Sampling is the heart of ReSTIR. At its core, it is a technique that takes multiple samples $(X_1, ..., X_M)$ produced according to a **source distribution $p$** and "resamples" them
...
and outputs one new sample $Y$ that is distributed proportionally to whatever function $\hat{p}$ we want.

Let's quickly dive into a concrete example applied to our many-light sampling problem:

Ultimately, when estimating direct lighting, we're trying to solve the following integral at each point $P$ in our scene:

\begin{equation}
L_o(P, \omega_o) = \int_{A}^{}f(P, \omega_o, \omega_i)V(P\leftrightarrow x)cos(\theta)dA_x
...
requires tracing a ray, an expensive operation).  The target function $\hat{p}$ needs not be too expensive to evaluate in practice or
...
\begin{equation}
\hat{p}(x) = f(P, \omega_o, \omega_i)cos(\theta)
...
- (Line 14) With these weights $w_i$, we are now going to choose a sample $X_i$ proportionally to its weight $w_i$: 
	...
	used to obtain the sample $X_i$ (it uses a non-precomputed CDF inversion technique for choosing that sample <d-cite key="PBRBookSampling1DFunction"></d-cite>). In this example, the probability
	of choosing $X_1$, $X_2$ or $X_3$ with the random index $s$ are then:
