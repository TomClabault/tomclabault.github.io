...
a buffer that represent the voxel-to-voxel visibility matrix of the scene or any suitable data structure).
...
1. Shadow rays russian roulette: if there is a 99% chance that 2 voxels are occluded, then 99% of the thime we may as well assume that the shadow ray is going 
to be occluded without even tracing it. 1% of the time we still trace the ray. This approach of using the visibility probability increases variance but should
...
For ReGIR, we're going to use the visibility probability directly in the grid-fill target function, to aid in sampling lights that are visible (similar to 2 in that respect):
