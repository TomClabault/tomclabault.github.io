...
for a given light may vray significantly within a grid cell so computing the visibility of the samples only, and always, from $$P_c$$ isn't great and this results in grid cell artifacts where noise increases
...
The recap of that visibility-term precision is that we should have more precision where lighting frequency is high: if visibility changes faster than the precision of the hash grid, then that where we
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>Le/d^2</td>
            <td>Le*cos(theta)/d^2</td>
            <td>Le*cos(theta)*cos(theta_L)/d^2</td>
            <td>Le*V*cos(theta)*cos(theta_L)/d^2</td>
            <td><br></td>
            <td><br></td>
        </tr>
        <tr>
...
Make a table to recap the convergence because ti's ahrd to eyeball at this point

Mention that with visibility, the position of the point matters a lot. Mention potential improvements section.
