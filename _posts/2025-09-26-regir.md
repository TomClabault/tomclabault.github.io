...
of {% cite conty2018ATS %} and SAH/SAOH are the cost functions used when building the tree. The SAOH is supposedly of higher quality but I'm getting suspiciously bad results in the white room scene
with the SAOH so that's why the SAH is going to be used in that scene instead of the SAOH in case it's a bug, to avoid defavoring the light tree. In every other scene however, the SAOH is noticeable improvement.
...
<script>
	...
	if (dataReGIRShadingResamplingParameter['imageBoxes'])
		new ImageBox(content, dataReGIRShadingResamplingParameter['imageBoxes'], 1918, 1038);
</script>
...
that's what it's meant for in the first place. Below are a few comparison images, all rendered at 1 SPP, 3 bounces, with the most efficient ReGIR settings found so far: $$L=32$$, $$R^N=64$$, $$R^C=12$$, $$S=4$$, light distributions enabled, with visibility
in the target function during shading-resampling and with BSDF MIS. Power sampling and light tree results are equal-time ish (rounded up as always for power sampling and light tree) with ReGIR.
