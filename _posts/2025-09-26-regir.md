...
## Improvements to visibility noise
	- Visibility in the target function at shading
		- Redo comparison functionof the parameter S because this changes everything and now higher S are more efficient

## Impact on GI quality

So in blog post, comparison screenshots were always rendered with 0 bounces to remove the variance introduced by GI sampling from the equation. ReGIR naturally (almost: BSDF term in the grid fill target function) extends to world space as
that's what it's meant for in the first place. Below are a few comparison images, all rendered at 1 SPP, 3 bounces (except for DI only), with the most efficient ReGIR settings found so far: $$L=32$$, $$R^N=64$$, $$R^C=12$$, $$S=4$$, light distributions enabled, with visibility
in the target function during shading-resampling and with BSDF MIS. Power sampling and light tree results are equal-time ish (rounded up as always for power sampling and light tree) with ReGIR and also 1SPP for reference.

As these comparisons combine all the features presented so far, they also serve as the reference for where the quality of this ReGIR implementation is at at the end of this blog post.
...
# Performance and memory usage
	- light distributions compaction
	- reducing reservoirs at later boucnes
		- careful about mirrors

# Limitations and potential improvements
	Mention that visibility term precision issue with lighting frequency being higher frequency thatn the grid, we need some adaptive grid precision or something, better cache placement
	- Better canonical candidates weigthing: NEE++ big degradation in quality only because of canonical samples	
...
# Have some of the image comparisons in annex
